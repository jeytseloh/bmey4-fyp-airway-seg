...
def compute_metrics(gt_path: str, pred_path: str, cl_path: str, save_csv: bool, 
    ...
    """
    pred_fids = os.listdir(pred_path) # {ATM/Aero}_{FID}_pred0.nii.gz, {ATM/Aero}_{FID}_pred0_wPt.nii.gz, {ATM/Aero}_{FID}_pt.pkl
    metrics = []
    ...
    for pred_fid in pred_fids:
        if pred_fid.endswith(".nii.gz") and not pred_fid.startswith("._"):
            ...
            if result is not None:
                ...
                if save_csv:
                    file_path = save_path + f"_{tsDataset}.csv"
                    file_exists = os.path.exists(file_path) and os.path.getsize(file_path) > 0
                    with open(save_path + f"_{tsDataset}.csv", 'a+', newline='') as f:
                        writer = csv.writer(f)
...
if __name__ == "__main__":
    ...
    tsDataset = ["Aero", "ATM"] # dataset used to evaluate model on {Aero, ATM22}
    ...
    pred_path_aero = save_path_aero = os.path.join(root_dir, "SAM-Med3D", "SAM-Med3D_results", model, "Aero_ori_spacing")
    ...
    pred_path_atm22 = save_path_atm22 = os.path.join(root_dir, "SAM-Med3D", "SAM-Med3D_results", model, "ATM22_ori_spacing")
