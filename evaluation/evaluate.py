...
def compute_metrics(gt_path: str, pred_path: str, cl_path: str, save_csv: bool, 
    ...
    for pred_fid in pred_fids:
        if pred_fid.endswith(".nii.gz") and not pred_fid.startswith("._"):
            ...
            if result is not None:
                ...
                if save_csv:
                    file_path = save_path + f"detail_{dataset}.csv"
                    file_exists = os.path.exists(file_path) and os.path.getsize(file_path) > 0
                    with open(save_path + f"detail_{dataset}.csv", 'a+', newline='') as f:
                        writer = csv.writer(f)
    ...
    with open(save_path + f"summary_{dataset}.txt", 'w') as f:
...
if __name__ == "__main__":
    ...
    trDataset = "Dataset111_ATM22Aero" # change this for different datasets
    ...
    pred_path_aero = save_path_aero = os.path.join(root_dir, "Results_v2", "Inference", trDataset, model, tsDataset[0])
    # pred_path_aero = save_path_aero = os.path.join(root_dir, "Testing", "naviairway_adam", trDataset, tsDataset[0])
    cl_path_aero = os.path.join(root_dir, "eval_dataset_full", "Aero_cl")
    compute_metrics(gt_path_aero, pred_path_aero, cl_path_aero, True, save_path_aero, trDataset)
    print(f"Evaluation on {tsDataset[0]} for {trDataset} done.")

    # evalute on ATM22 test dataset
    gt_path_atm22 = os.path.join(root_dir, "eval_dataset_full", "ATM22_gt")
    pred_path_atm22 = save_path_atm22 = os.path.join(root_dir, "Results_v2", "Inference", trDataset, model, tsDataset[1])
    # pred_path_atm22 = save_path_atm22 = os.path.join(root_dir, "Testing", "naviairway_adam", trDataset, tsDataset[1])
    cl_path_atm22 = os.path.join(root_dir, "eval_dataset_full", "ATM22_cl")
    compute_metrics(gt_path_atm22, pred_path_atm22, cl_path_atm22, True, save_path_atm22, trDataset)
    print(f"Evaluation on {tsDataset[1]} for {trDataset} done.")
