...
class nnUNetDatasetCL(object):
    def __init__(self, folder: str, case_identifiers: List[str] = None,
        ...
        for c in case_identifiers:
            ...
            self.dataset[c]['cl_file'] = join(folder, f"{c}_cl.npy") # CENTRELINE
    ...
    def load_case(self, key):
        ...
        if 'open_cl_file' in entry.keys():
            cl = entry['open_cl_file']
        elif isfile(entry['cl_file'][:-4] + ".npy"):
            cl = np.load(entry['cl_file'][:-4] + ".npy", 'r')

            if self.keep_files_open:
                self.dataset[key]['open_cl_file'] = cl

        return data, seg, entry['properties'], cl