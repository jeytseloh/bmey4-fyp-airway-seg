# https://github.com/MIC-DKFZ/nnUNet/blob/master/nnunetv2/preprocessing/preprocessors/default_preprocessor.py
...
class CentrelinePreprocessor(DefaultPreprocessor):
    """
    Custom preprocessor that processes the centreline of each target image additionally
    """
    ...
    def run_case_npy(self, data: np.ndarray, seg: Union[np.ndarray, None], properties: dict,
        ...
        # print(f"run_case_save unique val seg3: {np.unique(seg)}") # this is where it becomes [-1,0,1]
    ...
    def run_case_save(self, output_filename_truncated: str, image_files: List[str], seg_file: str,
        ...
        try:
            seg_cl = seg
            seg_cl[seg_cl == -1] = 0 # remove nonzero_label padding
            seg_cl = seg_cl.astype(np.uint16)

            cl = skeletonize_3d(np.squeeze(seg_cl)) # remove channel dimension
            cl = np.expand_dims(cl, axis=0)
            # check values of cl and convert to binary if necessary
            unique_values = np.unique(cl)
            if np.array_equal(unique_values, [0, 1]):
                print("binary")
                pass # already binary
            elif len(unique_values) == 2 and 0 in unique_values: # two unique vals and one is 0
                print(f"before conversion: {np.unique(cl)}")
                cl[cl != 0] = 1  # convert non-zero to 1
                print(f"after conversion: {np.unique(cl)}")
            elif len(unique_values) > 2 or not np.array_equal(unique_values, [0, 1]): # >2 unique vals or not binary
                raise ValueError(f"Array contains invalid values: {unique_values}. Array must be binary or convertible to binary.")
        except Exception as e:
            print(f"Error during skeletonization: {e}")

        np.save(output_filename_truncated + '_cl.npy', cl) # save cl as npy
